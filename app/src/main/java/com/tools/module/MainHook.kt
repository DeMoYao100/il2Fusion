...
class MainHook: IXposedHookLoadPackage {
        ...
        XposedHelpers.findAndHookMethod(
            ...
            object : XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam) {
                    ...
                    OverlayLogger.init(ctx)
                    OverlayLogger.append("${lpparam.packageName}已启动")
                    ...
                    // native initialize
                    try {
                        OverlayLogger.append("Native 初始化已开始")
    ...
    private fun startIl2cppWatcher() {
        Thread {
            repeat(60) {
                if (NativeBridge.isIl2cppLoaded()) {
                    OverlayLogger.append("已检测到 libil2cpp")
                    return@Thread
                }
                try {
                    Thread.sleep(500L)
